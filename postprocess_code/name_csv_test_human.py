...
def main(params):
    ...
    # sc_rna_path = f"/root/beifen/data/{dataset}/process_sc_rna_data.csv"  # [1289, 6025]
    sc_rna_path = join(data_dir, dataset, "process_sc_rna_data.csv")
    sc_rna = pd.read_csv(sc_rna_path)

    # cell_id_path = f"/root/beifen/data/{dataset}/cell_id.csv"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--metadata_path",
        default="../ST image/human/Wu_etal_2021_BRCA_scRNASeq/metadata.csv",
        type=str,
    )
    parser.add_argument(
        "--torch_path",
        default="test_1000_tangram_cca.pt",
        type=str,
    )
    parser.add_argument(
        "--result_path",
