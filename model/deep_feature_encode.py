...
class ImageEncoder(nn.Module):
    ...
    def forward(self, input_data):
        ...
        reconstruction_losses = torch.norm(torch.sign(input_data)*(reconstructed_data - input_data))

        total_sparse_penalty = torch.sqrt(torch.sum(torch.sum(self.decoder[0].weight**2, dim=1)))
