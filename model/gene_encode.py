...
class Encoder(nn.Module):
    def __init__(self, input_sizes, n_hidden, dim_z):
        ...
        self.encoders = nn.ModuleList([self.build_encoder(input_size, n_hidden) for input_size in input_sizes])
    ...
    def forward(self, input_data_list):
        encoded_data_list = [encoder(input_data) for encoder, input_data in zip(self.encoders, input_data_list)]
...
class SinglecellNet(nn.Module):
    def __init__(self, input_sizes, n_hidden, dim_z):
        ...
        self.decoders = nn.ModuleList([self.build_decoder(dim_z, output_size) for output_size in input_sizes])
    ...
    def forward(self, input_data_list,cluster_label_list):
        ...
        reconstructed_data_list = [decoder(latent) for decoder, latent in zip(self.decoders, latent_list)]
        
        # 计算重构损失和稀疏惩罚，这可能需要进一步定制
        total_reconstruction_loss =[torch.norm( (reconstructed_data - input_data)) \
                                    for input_data, reconstructed_data in zip(input_data_list, reconstructed_data_list)]

        total_reconstruction_loss = sum(total_reconstruction_loss)
        # print('total_reconstruction_loss',total_reconstruction_loss)
        total_sparse_penalty =[ torch.sqrt(torch.sum(torch.sum(decoder[0].weight**2, dim=1))) for decoder in self.decoders]
        ...
        return latent_list, reconstructed_data_list, total_reconstruction_loss, total_sparse_penalty,total_trip_loss_x
