...
image_resize = 0.1
cell_coordinates_csv = pd.read_csv(f'/home/yanghl/zhushijia/data/ST image/human/cells_on_spot.csv',\
...
cell_type_csv = pd.read_csv(f'plot_csv/{name}/{type}_spot_cell_type.csv',\
                            usecols=['cell_id'])

merged_df = pd.merge(cell_type_csv, cell_coordinates_csv, on='cell_id', how='left').dropna()
print(merged_df)
...
image = Image.open(f'/home/yanghl/zhushijia/data/ST image/human/CytAssist_FFPE_Mouse_Brain_Rep2_tissue_image.tif')
new_width = int(image.width * image_resize)
new_height = int(image.height * image_resize)
resized_image = image.resize((new_width, new_height))

resized_image.save(f'plot_csv/{name}/{type}_CytAssist_FFPE_Mouse_Brain_Rep1_tissue_image.jpg')
